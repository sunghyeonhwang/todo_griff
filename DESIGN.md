...
| 타임라인 스케일 | `HOUR_HEIGHT = 96px` (`PX_PER_MIN = 1.6`) → 하루 캔버스 2304px. 5분=8px, **10분=16px**, 15분=24px 전부 정수 |
| 스냅 2단계 | **저장 불변식 5분 배수** (에디터 입력 표현 가능) / **제스처(생성·이동·리사이즈) 10분** (토큰 `snapMinutes` 일치, 15→10 개정 §4.1) |
| 최소 블록 길이 | **창작 최소 20분 = 32px**(`MIN_DURATION`, mutation 경로가 강제) / **렌더 하한 24px**(`--block-min-height` = `RENDER_MIN_HEIGHT`, 레거시 15분 블록을 강제 변환 없이 가독 보장). 15→20 개정 §4.1 |
...
| 알림 | 시작 전 오프셋(0/5/10/15/30/60분) + **종료 알림 옵트인**(§7 개정). **앱이 열려 있는 동안만 동작** — UI에 정직하게 표기. 발화는 `lib/notify.ts` 어댑터(SW `showNotification` + 토스트) |
...
export const GESTURE_SNAP = 10;       // 토큰 --snap-minutes 일치(15→10 개정) — 96px/시에서 10분=16px 정수
export const STORE_SNAP = 5;          // 저장 단위(분)
export const MINOR_GRID = 10;         // 보조 헤어라인 간격(분) = 16px — 10분 스냅 시각 정합
export const MIN_DURATION = 20;       // 창작 최소 블록 길이(분) = 32px — mutation 경로가 강제(15→20 개정)
export const RENDER_MIN_HEIGHT = 24;  // 카드 렌더 하한(px) = 토큰 --block-min-height — 레거시 15분 블록 가독 보장(강제 변환 금지)
...
96px/시는 5분=8px 정수이고, 10분=16px·15분=24px 전부 정수다.
(더 촘촘한 밀도를 원하면 `HOUR_HEIGHT` 상수 1곳 변경이지만, 최소 블록 렌더 규칙 재조정 필요.)

**개정(2026-07-18) — 스냅 10분 · 보조 눈금 · 최소 블록 20분:**
- **제스처 스냅 15→10분**(`GESTURE_SNAP`·토큰 `--snap-minutes`·`tokens.ts snapMinutes` 동기): 96px/시에서 10분=16px 정수라 서브픽셀 없음. 생성·이동·리사이즈가 상수 공유로 일괄 10분 스냅된다.
- **10분 보조 헤어라인 도입**(위 "부(副) 눈금 없음" 규칙 개정): 10분 스냅과의 시각 정합을 위해 시간선 사이 10/20/30/40/50분에 옅은 눈금(같은 `--surface-timeline-line`을 opacity 40%로 파생 — 별도 토큰 없이 계층만 구분). 라벨은 여전히 시간선(정시)에만.
- **최소 블록 15→20분**(`MIN_DURATION`, =32px): 10분 스냅과 어긋나던 15분 최소를 정리하고 제목 렌더 여유 확보. **단, 창작 최소와 렌더 하한을 분리한다** — `MIN_DURATION`(20분)은 `normalizeRange`(add/resize 등 mutation 경로)만 강제하고, 로드용 `sanitizeRange`는 최소 길이 floor를 적용하지 않아 **기존 15분 블록을 20분으로 강제 변환하지 않는다**(데이터 무손실). 대신 카드가 `RENDER_MIN_HEIGHT`(=토큰 `blockMinHeight` 24px)를 `min-height`로 보장해 짧은 레거시 블록도 가독성을 유지한다. 즉 토큰 `blockMinHeight`의 의미가 "최소 블록 길이 일치"에서 "**카드 렌더 하한**"으로 개정됐다.
...
- 시간 라벨 **25개(00:00–24:00)**, 전부 `formatMinutes(h*60)`으로 생성. 색 `var(--surface-timeline-label)`, 정시 그리드 헤어라인 `var(--surface-timeline-line)`. **10분 보조 헤어라인**(개정): 시간선 사이 10/20/30/40/50분에 같은 색을 opacity 40%로 옅게 그린다(부 눈금 부활 — 위 §4.1 개정). 라벨은 정시에만, 그리드라인에 수직 중앙 정렬(`translateY(-50%)`), `--fs-xs`(11px) tabular-nums.
...
- 체크박스: 우측 24px 원형(히트 영역 44×44px, Apple HIG), `data-no-dnd` + `stopPropagation`. 에디터 안 열림, 즉시 커밋(명시적 저장 원칙의 예외 — KR 진척 ±(§15.4)와 함께 둘뿐).
...
- **레이아웃(Structured 아나토미 — 사용자 제공 스크린샷 기준)**:
  1. **컬러 헤더 밴드**(`var(--blk-solid)` 풀블리드): 좌상단 X 닫기 원(반투명 흰 `rgba(255,255,255,.25)` + `text-on-solid`) · 대형 아이콘 배지(80px 반투명 원 `rgba(255,255,255,.25)`, 안에 아이콘 `<img>` 64px, 탭 = 피커 카드 토글 — surface-card 링 없음, 흰/검 테두리 제거 2026-07-08). **밴드 위 컨트롤(X·완료 원·제목 포커스 밑줄)은 테마 무관 흰 계열로 고정** — surface-card를 쓰면 다크에서 검은 원이 떠 보인다(2026-07-10). 완료 원 = `border-white/70`, 완료 시 `bg-white + --blk-solid 체크` · 시간 요약 "10:00 ~ 10:15 (15분)"(라이브 갱신) · **밑줄 제목 input**(투명 bg, `text-on-solid`, `--fs-lg`; create 모드만 autoFocus — iOS는 열기 제스처 핸들러에서 동기 설정해야 키보드 뜸) · (edit) 우측 완료 원(폼 로컬 토글, 저장 시 반영). **빈 제목 저장 시 '새 일정' 자동 대체 — 제목 때문에 저장이 비활성화되는 일 없음**
...
- **저장 시맨틱: 명시적 저장.** 스토어 변이가 정확히 1회(add/update), 검증 초크포인트 1곳, 취소가 자명하게 올바름. 예외는 카드 위 체크박스(§4.9)와 OKR의 KR 진척 ±(§15.4) 둘뿐.
...
> 2026-07-10: 하단 탭 바(오늘|목표) 신설로 앱이 2-뷰가 됐다 — 상세는 §15.1. 아래 서술은 '오늘' 탭 기준.
...
- 발화 조건: `dateKey === 오늘 && !completed`이고 `fireAt ≤ nowMinutes() ≤ fireAt + 10분`(늦은 발화 허용창 — 그 이상 지난 건 조용히 드롭, 몇 시간 뒤 발화 금지). `fireAt`은 발화 종류별로 다르다(아래 개정).
- 중복 방지: `firedKey`를 **영속** `firedAlarms`에 기록 — 새로고침해도 재발화 없음. 시간/오프셋 수정 시 키가 바뀌어 자동 재무장.
- **발화 경로 (중요)**: `new Notification()` 생성자는 **Chrome for Android에서 TypeError**를 던짐. 규범 경로: SW가 등록돼 있으면(프로덕션은 vite-plugin-pwa가 보장) `reg.showNotification(title, { body, tag: firedKey, icon: '/pwa-192.png' })`. SW 없는 환경(dev)은 try/catch로 `new Notification` 폴백. **어느 경로든 인앱 토스트는 항상 병행**. — 이 경로 전체를 **`lib/notify.ts` 어댑터**로 추상화했다(아래 개정).
- 권한: 로드 시 요청 금지. 에디터에서 알림(시작/종료) 최초 켜는 사용자 제스처 안에서만 요청.

**개정(2026-07-18) — 알림 어댑터 분리 · 종료 알림 · 진행 중 카운트다운:**
- **알림 어댑터 `lib/notify.ts` 신설**: 발화 경로(SW `showNotification` + 인앱 토스트 병행)를 `notify(title, body, tag)` 하나로 추상화. `alarms.ts`가 유일 소비자이며 발화 방식을 모른다. **⚠ 향후 네이티브 앱(Capacitor 로컬 알림) 전환 시 이 파일 1곳만 교체**한다(2026-07-18 사용자 확정: 네이티브 앱 추후 트랙, 데이터는 Que Supabase 공유, PWA 푸시 서버는 하지 않음).
- **종료 알림(옵트인)**: `TimeBlock.endAlarm?: boolean`(에디터 알림 카드의 "종료 알림" 토글). 발화 `fireAt = endMin`. `firedKey = id|dateKey|end|fireAtMin`(시작 키 `id|dateKey|fireAtMin`과 `end` 구분자로 충돌 금지 — 두 키 모두 `split('|')[1] === dateKey`라 `pruneFiredAlarms`가 그대로 파싱). 시작 알림과 동일한 10분 늦은 발화창·영속 dedup·notify 경로. `endAlarm`은 `true`일 때만 저장(가법적, 버전업 불요 — sanitize가 미지 필드 보존 §13.2).
- **진행 중 블록 카운트다운**: 현재 시각이 블록 범위(`startMin ≤ now < endMin`) 안이고 미완료면 카드에 "남은 24분" 소형 표시. **나우라인의 `useNow` 30초 틱 인프라를 재사용**(새 타이머 없음 — 카드가 `useNow` 구독). 완료 블록엔 미표시.
...
│   ├── TabBar.tsx            # 하단 탭 2개(오늘/목표) — §6.5·§15
│   ├── OkrScreen.tsx         # 목표(OKR) 화면 — 분기 내비+목표 카드+교육형 빈 상태(§15)
│   ├── OkrEditor.tsx         # 목표/KR 생성·편집 시트(BottomSheet 재사용, §15)
...
│   ├── DayReviewSheet.tsx    # 하루 마감 See 카드 시트 — 계획 vs 실행(§16)
├── hooks/
│   ├── useNow.ts             # {nowMin, todayKey} 30초 틱 + visibilitychange
│   ├── useDaySnapshot.ts     # 그날 첫 조회 시 계획 스냅샷 캡처(§16, 양 스토어 하이드레이션 게이트)
...
│   ├── uiStore.ts            # 날짜·하단 탭·에디터·선택·리뷰·알림권한·토스트 (드래그 프리뷰 없음)
│   ├── reviewStore.ts        # persist(v1, dayblocks:review) — 계획 스냅샷 변이 초크포인트(§16)
│   └── okrStore.ts           # persist(v1, dayblocks:okr) — OKR 변이 초크포인트(§15)
└── lib/
    ├── time.ts               # 지오메트리 상수(tokens.ts 참조)·변환·스냅·nowMinutes·formatMinutes + date-fns 래퍼
    ├── tokens.ts             # design_token/tokens.ts 사본 (BLOCK_COLORS/LAYOUT/MOTION/Z_INDEX; hourHeight=96·snapMinutes=10 수정)
    ...
    ├── alarms.ts             # 30초 폴링 스케줄러(시작·종료 알림) + 권한 요청 — 발화는 notify.ts 경유(§7)
    ├── notify.ts             # 발화 어댑터 notify(title,body,tag) — SW showNotification+토스트, 네이티브 전환 교체점(§7)
...
모든 인증 호출 공통 헤더: `Authorization: Bearer <token>`, `X-Que-Via: mobile`(Que `auth.ts`가 `mobile`을 화이트리스트에 포함 → ChangeLog `via=mobile` 기록), 본문 있으면 `Content-Type: application/json`. **CORS 기확보**: Que가 `https://todo.griff.co.kr`·`http://localhost:5173`·`:3000`을 화이트리스트에 등록, 허용 헤더 `Authorization, Content-Type, X-Que-Via`, 메서드 `GET/POST/PATCH/DELETE/OPTIONS`(프리플라이트 커버), credentials 없음(Bearer 방식). 커스텀 헤더(`X-Que-Via`)로 프리플라이트가 발생하지만 서버가 처리한다. **예외 — SSO 표면(§14.9):** `GET /api/auth/sso` 한 곳만 쿠키 기반(`credentials:'include'`)이라 이 경로의 응답은 `Access-Control-Allow-Credentials: true` + 와일드카드 아닌 정확한 오리진 반향이 필요하다. 이 경로는 `X-Que-Via`를 붙이지 않아(아래 근거) simple request로 유지된다.
...
- **에러 등급:** `401`(토큰 만료/무효) → authStore를 `anon`으로 전이 + 로그인 유도, 아웃박스 보존(재로그인 후 플러시). `403 NOT_AUTHORIZED`·`404 NOT_FOUND`(원격에서 삭제/권한상실) → `syncState='error'` + 토스트(재시도 안 함, 사용자 개입). `5xx`·네트워크 실패 → `syncState='pending'` **지수 백오프 재시도**: 재시도성 실패가 연속되면 주기 틱(30초) 플러시를 30s→1m→2m→…(상한 10분)으로 늦춘다. 사용자 명시 행동(수동 새로고침·`online` 복귀·재로그인)은 백오프를 리셋하고 즉시 시도한다.
- **원격 소실(삭제·취소·병합·재배정):** 풀 성공 시 응답에 없는 태스크의 연동 블록은 **지우지 않고 링크만 해제**해 로컬 전용 블록으로 강등한다(로컬 우선·비파괴 §14.1) + 잔여 아웃박스 op 폐기 + "Que에서 사라진 할 일 N개…" 토스트. 오프라인/실패 풀에서는 실행하지 않는다(성공 응답 기반 판정만).
- **로컬 삭제:** 연동 블록을 로컬에서 삭제하면 그 블록의 잔여 아웃박스 op를 폐기한다 — 삭제는 write-back 대상이 아니므로(§14.0) 삭제된 블록의 마지막 move/status가 Que로 발사되면 안 된다. flush도 실행 직전 블록 실재를 재확인한다(이중 안전).
...
### 14.9 SSO 자동 연결 (쿠키 세션 재사용)

que.griff.co.kr에 이미 로그인한 브라우저가 todo.griff.co.kr을 열면 email/password 재입력 없이 자동 연결된다. §14.8 email/password 로그인과 공존하는 보조 진입로다.

- **엔드포인트:** `GET /api/auth/sso`(`queApi.ssoLogin()` — fetch 단일 통과점 유지 §14.8). **쿠키 기반**(`credentials:'include'`)으로 Que 세션 쿠키를 실어 보내고 성공 시 `{token, user:{id,name,role}}`를 받는다(로그인과 동일 페이로드). **이 표면만 예외적으로 Bearer도 `X-Que-Via`도 붙이지 않는다** — 커스텀 헤더가 없으면 프리플라이트 없는 simple request로 남아 credentials 흐름이 단순해지기 때문(CORS 요건은 §14.2 예외 참조).
- **silent 강등:** 부팅 자동 시도라 미로그인(401)·비200·기형 200·네트워크 오류는 **throw 없이 `null`**로 강등한다(로그인 화면 플로우를 깨지 않음). `ssoLogin()`은 예외를 던지지 않는다.
- **부팅 트리거(자동 1회):** 앱 진입 시 로그인 화면을 그릴 상황(`status==='anon' && !dismissed`)이면 `authStore.trySso()`를 **1회** 조용히 시도한다(`ssoAttempted` 휘발 플래그로 중복 가드, StrictMode 이중 실행도 이 가드가 흡수). 시도가 끝나기 전에는 로그인 폼 대신 최소 게이트(다크 스피너 + `sso.connecting`)를 보여 성공 시 폼 깜빡임을 막는다 — 성공은 셸로 직행, 실패/미로그인은 게이트를 닫고 기존 LoginScreen.
- **성공 전이:** `login()`과 동일하게 `{token, user, status:'authed', dismissed:false}`로 전이 + 기존 연결 토스트(`STRINGS.que.toast.connected`) 재사용.
- **수동 재연결:** LoginScreen에 "Que 세션으로 연결" 보조 버튼(`trySso(manual=true)`)을 둔다. 수동은 `ssoAttempted` 가드를 우회해 재시도하고, 실패 시에만 안내 토스트(`STRINGS.que.sso.notFound`)를 띄운다(부팅 자동 실패는 무음).
- **로그아웃 존중:** "나중에"(dismiss)·명시 로그아웃 사용자는 **자동 SSO를 돌리지 않는다.** `reconnect`도 자동 재로그인하지 않고 로그인 화면으로 돌려보내며, 재연결은 위 수동 버튼(또는 email/password)으로만 트리거한다 — 로그아웃 직후 자동 재로그인 금지(사용자 의사 존중, 확정 정책).
- **카피(규칙5):** `STRINGS.que.sso.{button, connecting, notFound}` 추가. 성공 토스트는 기존 `toast.connected` 재사용.

### 14.10 개인 블록·See 스냅샷 서버 동기화 (`store/personalSync.ts`)

> 상태: **구현 완료(2026-07-19).** §14의 로컬 우선·아웃박스·백오프 규범을 **두 번째 동기 축**으로 확장한다.
> §14.1~§14.7이 `queTaskId` **있는** 연동 블록의 Que Task 라이트백(queSync)이라면, 여기는 `queTaskId` **없는** 순수 개인 블록과 See 스냅샷을 Que 개인 저장소(`/api/blocks`·`/api/day-reviews`)에 미러링한다. **두 축은 겹치지 않는다** — 연동 블록은 개인 동기에서 제외(이중 저장 금지), 개인 블록은 tasks 라이트백에서 제외.

- **철학(불변, §14.1 계승):** localStorage=진실, 서버=백업 미러. 미로그인/오프라인이면 동기화 전면 무동작이고 앱은 순수 로컬 플래너로 완전 동작한다(`status!=='authed'` 게이트).
- **Que API 계약(구현분):** `GET /api/blocks?from&to`(≤31일) → `{blocks:[{id,dateKey,payload,updatedAt}]}` · `PUT /api/blocks {blocks:[{id,dateKey,payload}]}`(≤100건, payload 8KB) → `{saved,rejected}` · `DELETE /api/blocks?ids=a,b,c`(≤100) · `GET/PUT /api/day-reviews`(≤31건, 16KB). 인증은 기존 PAT Bearer(queApi 래퍼 재사용), 401→`authStore.expire()`.
- **payload:** `TimeBlock` 전체를 payload로 직렬화하되 **`queTaskId`·`syncState`는 방어적으로 제외**(개인 블록엔 없어야 하지만 명시적으로 뺌). 8KB 초과 블록은 스킵+콘솔 경고(정상 블록은 수백 바이트라 실질 미발생). LWW 비교 기준은 payload 내부 `updatedAt`(편집 시각)이다 — 서버 레코드 `updatedAt`(수신 시각)이 아님(§14.7 편집시각 LWW 계승).
- **풀(pull):** 부팅·재로그인 시 최근 14일 블록 GET → **id 단위 LWW 병합**(payload.updatedAt vs 로컬 `updatedAt`, 원격이 최신일 때만 갱신). **로컬에 미전송 변경(아웃박스 잔존)이 있으면 무조건 로컬 승**(hasBlockOp 가드). 로컬이 연동 블록이면 개인 미러로 덮지 않는다. 신규 블록은 **서버 id를 보존해 재생성**(`addBlock`에 optional `id` 추가 — 로컬 id=동기화 키라 원격 복원 시 보존 필수). `applyingRemote` 플래그가 풀 적용을 라이트백 에코에서 차단(§14.6 패턴 재사용). 스냅샷은 최근 7일 GET → **로컬에 없는 날만 채움**(불변이라 충돌 없음 — `reviewStore.applyRemoteSnapshot`, `capturedAt` 원본 보존).
- **푸시(push):** `blocksStore` 구독 → **개인 블록만** 아웃박스(`queTaskId` 있으면 제외). blockId 단위 코얼레스(upsert/delete 최신 의도만). 신규·내용변경(`updatedAt` bump)·연동해제 전환 → upsert, 로컬 삭제 → delete, 개인→연동 전환 → delete(개인 저장소에서 제거). 30초 주기 플러시·지수 백오프(30s→10m)·오프라인 보존 모두 §14.7 규범 그대로. 스냅샷 생성(`reviewStore` 구독)은 불변이라 PUT 1회면 충분(재시도만).
- **rejected 처리(§14.7 재시도성 분류):** `PUT /api/blocks`의 `rejected`(남의 id 충돌 등 — 실질 uuid라 미발생)는 재시도 무의미 → 콘솔 경고+op 드롭(비재시도 등급). 4xx(413 payload 초과 포함)도 드롭, DELETE 404는 삭제 완료로 간주, 5xx·네트워크만 아웃박스 유지+백오프.
- **초크포인트 준수:** 블록 변이는 여전히 `blocksStore` 액션(`addBlock`/`updateBlock`)만 경유. `addBlock`에 optional `id`를 **가법적**으로 더한 것 외 신규 변이 액션 없음. 리뷰는 `reviewStore.applyRemoteSnapshot`(불변·채우기 전용) 1개 추가. 네트워크는 `queApi`에만. `personalSync`는 상태를 읽고 아웃박스/전이만 관리한다.
- **신설/변경:** `store/personalSync.ts`(신설, 동기 레이어) · `lib/queApi.ts`(getBlocks/putBlocks/deleteBlocks/getDayReviews/putDayReviews + 타입) · `store/blocksStore.ts`(NewBlockInput.id, addBlock 보존) · `store/reviewStore.ts`(applyRemoteSnapshot) · `main.tsx`(startPersonalSync 배선). 동기화는 무음(사용자 토스트 없음 — 콘솔 경고만), 신규 카피 없음.

---

## 15. OKR — 분기 목표 트래커 (D-4)

> 상태: **구현 완료(2026-07-10, 사용자 결정: 하단 탭 2개 + MVP 목표 트래커만).**
> **앱 로컬 전용** — Que에 OKR 개념이 없으므로 동기화·write-back 없음(Que 무변경). 블록↔목표 연결은 2차 백로그.

### 15.1 IA — 하단 탭 바 (§6.5 확장)

- 앱이 처음으로 **2-뷰**가 된다: **오늘**(기존 타임라인: DateHeader+인박스+Timeline+FAB) | **목표**(OkrScreen). `TabBar.tsx`가 셸 컬럼 하단 `shrink-0` 형제로 전환을 담당(달력/과녁 아이콘 + 라벨, 활성=accent-primary, 터치 56px, `safe-area-inset-bottom` 흡수).
- 탭 상태는 uiStore `activeTab`(**휘발** — 앱 시작은 항상 '오늘'). 비활성 화면은 언마운트.
- AddFab은 오늘 탭 전용, `bottom = 탭 바 높이(56px)+20px`로 상향(겹침 방지).

### 15.2 데이터 모델 (types.ts)

```ts
Objective { id, quarter /* 'YYYY-Qn' */, title, createdAt, updatedAt }
KeyResult { id, objectiveId, title, target /* >0 */, current /* ≥0 */, unit, createdAt, updatedAt }
```
- 진행률: KR = `clamp(current/target, 0..1)`(target 초과 입력 허용, 표시만 100% 클램프). Objective = 소속 KR 평균(KR 없으면 0). 파생 헬퍼 `krProgress`/`objectiveProgress`는 okrStore의 순수 export.
- 분기 유틸은 `lib/time.ts`(`quarterKey`/`shiftQuarter`/`quarterLabel`).

### 15.3 스토어 — `store/okrStore.ts` (persist `dayblocks:okr`, v1)

- §3.1 규율 계승: **이 액션들이 OKR 데이터의 유일한 변이 초크포인트** — addObjective/updateObjective/deleteObjective(소속 KR 연쇄 삭제)/addKeyResult(고아 생성 차단)/updateKeyResult/deleteKeyResult/bumpKeyResult(±1).
- 불변식은 스토어가 강제: 빈 제목 → '새 목표'/'새 핵심 결과', target은 0보다 큰 유한수(아니면 1 보정), current ≥ 0. 매 하이드레이션 sanitize(merge) — 잘못된 quarter 드롭, **고아 KR(소속 Objective 없음) 드롭**, safeStorage 계약(§3.1) 그대로.

### 15.4 화면 — `OkrScreen.tsx` + `OkrEditor.tsx`

- **분기 내비**: ‹ 2026년 3분기 › (화면 로컬 state, 초기=현재 분기). 분기별 목표 목록(createdAt 순).
- **목표 카드**: 제목(탭=편집) + 진행률 %(accent) + 진행률 바(`role="progressbar"`) + KR 행들 + '핵심 결과 추가'.
- **KR 행**: 제목·`current / target unit · pct%`(탭=편집) + **− / + 버튼**(`bumpKeyResult` 즉시 커밋 — §4.9 체크박스와 동급의 명시적-저장 예외, −는 current=0에서 비활성).
- **OkrEditor**(BottomSheet 재사용, §5 크롬): 케이스 4개(목표 생성/편집·KR 생성/편집)를 한 시트로. 저장 = 스토어 변이 정확히 1회(명시적 저장). 삭제 = 탭-어게인 확인(§5). end-미만-start류 비활성 조건은 KR의 `target > 0` 하나. 진입점이 OkrScreen 하나라 에디터 상태는 **화면 로컬 state**(uiStore 아님 — §3.2 취지).
- **교육형 빈 상태(첫 사용 가이드 — 로드맵 E-F 요건 필수)**: "OKR이란?"을 왜→무엇→어떻게→예시 순 평이한 우리말로(`STRINGS.okr.empty*`).
- 카피는 전부 `STRINGS.okr`·`STRINGS.tabs`(규칙5).

### 15.5 신설 파일

| 파일 | 역할 |
| --- | --- |
| `src/store/okrStore.ts` | persist(v1, dayblocks:okr) + 변이 초크포인트 + sanitize + 진행률 헬퍼 |
| `src/components/TabBar.tsx` | 하단 탭 2개(오늘/목표) — §6.5 |
| `src/components/OkrScreen.tsx` | 분기 내비 + 목표 카드 + 교육형 빈 상태 |
| `src/components/OkrEditor.tsx` | 목표/KR 생성·편집 시트(BottomSheet 재사용) |

---

## 16. 하루 마감 See 카드 (Plan-Do-See의 See)

> 상태: **구현 완료(2026-07-18).** 하루를 계획(Plan)하고 실행(Do)한 뒤, 저녁에 계획 대비 실행을 돌아보는(See) 개인 회고 카드.
> **개인 시간 회고 전용** — Que 팀 회고와 분업한다(이 카드는 Que에 동기화하지 않는다). 조회 전용.

### 16.1 계획 스냅샷 (`store/reviewStore.ts`, persist `dayblocks:review` v1)

- **그날 첫 조회 시점**(persist 하이드레이션 후 + dateKey 전환 감지)에 오늘 블록들의 `{id, title, startMin, endMin}` 목록을 자동 저장한다. 스냅샷은 **날짜당 1회·불변**(첫 조회 시점의 '계획' 고정) — `captureSnapshot`이 이미 있으면 no-op.
- §3.1 규율 계승: 스냅샷 데이터의 **유일한 변이 초크포인트는 이 스토어 액션**이다. 컴포넌트/훅의 localStorage 직접 접근 금지 — `safeStorage` 어댑터 1곳만 통과. 매 하이드레이션 sanitize(구조 가드 실패 드롭).
- **최근 7일만 보관**(용량) — `captureSnapshot`·sanitize가 dateKey 내림차순 상위 7개만 유지.
- **캡처 트리거**: `hooks/useDaySnapshot.ts` — `useNow`의 `todayKey`를 구독, 마운트(하이드레이션 후) + 자정 롤오버 시 캡처. `blocksStore`·`reviewStore` **둘 다 하이드레이션 완료**를 보장한 뒤 캡처(하이드레이션 전 캡처가 직후 merge로 덮이는 것 방지).
- **⚠ 향후 Supabase 저장 전환 예정**(사용자 결정): 스냅샷 로직을 `reviewStore` 액션으로 격리해 교체 지점을 명확히 했다 — 저장 백엔드가 바뀌어도 `captureSnapshot` 시그니처는 유지.

### 16.2 See 카드 (`components/DayReviewSheet.tsx`, BottomSheet 재사용)

- **진입점**: `DateHeader`의 아이콘 버튼(항상 접근 가능). **저녁(기본 18:00 이후) & 오늘**이면 도트로 강조(`useNow` 재사용). `uiStore.reviewOpen`(휘발)로 열고, 앱 전체 인스턴스 1개. 활성 날짜(`activeDateKey`)의 스냅샷을 본다.
- **내용(조회 전용)**: 계획 실행률 바(완료된 계획 / 계획 수 %) + 계획 N개·N시간 vs 완료 M개·M시간 2열 + 계획에 없던 추가 블록 수(스냅샷에 없던 id) + 밀린(살아있고 미완료인 계획) 목록.
- **빈 상태**: 그 날짜의 스냅샷이 없으면(앱을 안 켠 과거 등) 교육형 안내. 데이터 변이 없음.

### 16.3 모델·신설 파일

- `TimeBlock`에 추가 필드 없음(스냅샷은 별도 스토어). 카피는 전부 `STRINGS.review`(규칙5).

| 파일 | 역할 |
| --- | --- |
| `src/store/reviewStore.ts` | persist(v1, dayblocks:review) + `captureSnapshot`(불변·7일 정리) + sanitize — 스냅샷 변이 초크포인트(Supabase 전환 격리점) |
| `src/hooks/useDaySnapshot.ts` | 그날 첫 조회 시 스냅샷 캡처(양 스토어 하이드레이션 게이트 + 자정 롤오버) |
| `src/components/DayReviewSheet.tsx` | See 카드 시트(BottomSheet 재사용) — 계획 vs 실행 파생 지표 |
