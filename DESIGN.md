...
| 항목 | 결정 |
|---|---|
| 스택 | Vite 7 + React 19 + TypeScript 5.9, Tailwind CSS **v4**(CSS-first), Zustand 5, date-fns 4, @dnd-kit/core 6 + modifiers, vite-plugin-pwa |
...
- `design_token/tokens.css` → `src/styles/tokens.css`로 복사(그대로 사용). 라이트 기본 + `[data-theme="dark"]` + media 폴백(`:root:not([data-theme="light"])`)이 이미 구현되어 있음.
